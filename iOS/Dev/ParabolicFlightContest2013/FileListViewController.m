...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // FileContentViewController *fileDetail = FileList[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsDirectory = [paths objectAtIndex:0];
    NSString *logFilePath =[documentsDirectory stringByAppendingPathComponent:FileList[indexPath.row]];
    
    NSLog(@"%@", logFilePath);
    NSLog(@"%@ %@ xx",documentsDirectory,logFilePath);
    NSLog(@"ok =) =)");
    
    self.fileDetail = [[FileContentViewController alloc]  initWithNibName:@"FileContentViewController" bundle:nil];
    self.fileDetail.filePath = logFilePath;
    
    [[self navigationController] pushViewController:self.fileDetail animated:NO];
